...
def merge_config(args, file_config: dict) -> dict:
    ...
    return {
        ...
        "device_port": (
            ...
            else file_config.get("device_port", DEFAULT_DEVICE_PORT)
        ),
        "appium_ip": (
            args.appium_ip
            if args.appium_ip is not None
            else file_config.get("appium_ip", DEFAULT_APPIUM_IP)
        ),
        "appium_port": (
            args.appium_port
            if args.appium_port is not None
            else file_config.get("appium_port", DEFAULT_APPIUM_PORT)
        ),
        ...
        "button_text": (
            ...
            else file_config.get("button_text", DEFAULT_BUTTON_TEXT)
        ),
        "button_id": (
            args.button_id
            if args.button_id is not None
            else file_config.get("button_id")
        ),
        "button_xpath": (
            args.button_xpath
            if args.button_xpath is not None
            else file_config.get("button_xpath")
        ),
        "interval": (
            args.interval
            if args.interval is not None
            else file_config.get("interval", DEFAULT_INTERVAL)
        ),
        "timeout": (
            args.timeout
        ...
        "screenshot_dir": (
            ...
            else file_config.get("screenshot_dir", SCREENSHOT_DIR)
        ),
...
def parse_arguments():
    ...
    parser.add_argument(
        "--device-ip", type=str, required=True, help="Android设备的IP地址"
    )
    parser.add_argument(
        "--device-port", type=str, default=None, help="Android设备的adb端口，默认为5555"
    )
    parser.add_argument(
        "--appium-ip",
        type=str,
        default=None,
        help="Appium服务器的IP地址，默认为127.0.0.1",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='要点击的按钮文本，默认为"领取成功"',
    )
    parser.add_argument("--button-id", type=str, default=None, help="要点击的按钮ID")
    parser.add_argument(
        "--button-xpath", type=str, default=None, help="要点击的按钮XPath"
    )
    parser.add_argument(
        "--interval", type=int, default=None, help="检测间隔（秒），默认为2秒"
    )
    parser.add_argument(
        "--timeout",
        type=int,
        default=None,
        help="检测超时时间（秒），默认为300秒，设为0表示不超时",
    )
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if attempt < MAX_RETRIES:
                logger.warning(
                    f"连接失败（第 {attempt}/{MAX_RETRIES} 次），{RETRY_DELAY} 秒后重试: {e}"
                )
    ...
    while not _shutdown_requested:
        ...
        try:
            ...
            if not elements and button_xpath:
                logger.debug(
                    f"第 {attempt_count} 次尝试通过XPath查找按钮: {button_xpath}"
                )
                elements = driver.find_elements(AppiumBy.XPATH, button_xpath)

            if not elements and button_text:
                logger.debug(
                    f"第 {attempt_count} 次尝试通过文本查找按钮: {button_text}"
                )
                elements = driver.find_elements(
                    AppiumBy.ANDROID_UIAUTOMATOR, f'text("{button_text}")'
                )
            ...
            if elements:
                ...
                if screenshot_enabled:
                    take_screenshot(
                        driver, screenshot_dir, f"{screenshot_seq:04d}_success"
                    )
                return True
            else:
