...
def process_images(
    input_dir: str, output_dir: str, frame_path: str, max_workers: int = 4
) -> tuple[int, int]:
    """处理目录中的所有图片。
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        # 提交所有任务
        future_to_image = {
            executor.submit(add_frame_to_image, input_path, frame_path, output_path): (
                input_path,
                output_path,
            )
            for input_path, frame_path, output_path in images_to_process
        }
        ...
        for future in concurrent.futures.as_completed(future_to_image):
            ...
            try:
                ...
                if result:
                    success += 1
                    logging.info(
                        f"已处理: {os.path.basename(input_path)} -> {os.path.basename(output_path)}"
                    )
                else:
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "-i", "--input", default="images", help="输入图片目录 (默认: images)"
    )
    parser.add_argument(
        "-o", "--output", default="images_new", help="输出图片目录 (默认: images_new)"
    )
    parser.add_argument(
        "-f",
        "--frame",
        default="kuangjia.psd",
        help="边框图片路径 (默认: kuangjia.psd)",
    )
    parser.add_argument(
        "-w", "--workers", type=int, default=4, help="最大线程数 (默认: 4)"
    )
...
def main() -> int:
    ...
    frame_path = (
        os.path.join(script_dir, args.frame)
        if not os.path.isabs(args.frame)
        else args.frame
    )
