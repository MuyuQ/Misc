...
def download_and_convert_image(url: str, filename: str, output_dir: str = 'images', timeout: int = 30, retries: int = 3) -> bool:
    """下载并保存图片（必要时进行模式转换）。
    ...
    for attempt in range(retries + 1):
        try:
            ...
            if attempt < retries:
                logging.warning(f'下载图片失败 {url} (尝试 {attempt + 1}/{retries}): {req_err}')
                time.sleep(2 ** attempt)  # 指数退避
            else:
                logging.error(f'下载图片失败 {url}: {req_err}')
        except Exception as e:
            if attempt < retries:
                logging.warning(f'处理图片失败 {url} (尝试 {attempt + 1}/{retries}): {e}')
                time.sleep(2 ** attempt)  # 指数退避
            else:
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("-f", "--file", default="ddddd.xlsx",
                        help="Excel文件路径 (默认: ddddd.xlsx)")
    parser.add_argument("-s", "--sheet", default="商品详情",
                        help="工作表名称 (默认: 商品详情)")
    parser.add_argument("-o", "--output", default="images",
                        help="图片输出目录 (默认: images)")
    parser.add_argument("-c", "--column-url", default="图片链接",
                        help="图片URL列名 (默认: 图片链接)")
    parser.add_argument("-n", "--column-name", default="spu",
                        help="文件名列名 (默认: spu)")
...
def download_images_concurrent(df: pd.DataFrame, url_column: str, name_column: str, output_dir: str, workers: int, timeout: int, retries: int) -> tuple[int, int]:
    """并发下载图片。
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        for index, row in df.iterrows():
            ...
            if pd.isna(img_url) or pd.isna(new_filename):
                continue
            future = executor.submit(download_and_convert_image, str(img_url), str(new_filename), output_dir, timeout, retries)
...
def main() -> int:
    ...
    try:
        ...
        total, success = download_images_concurrent(
            df, args.column_url, args.column_name, args.output, 
